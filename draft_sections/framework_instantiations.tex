...
prior).
On this recurrent backbone the mechanism is neutral
(Section~\ref{sec:frameworkresults}): the gated recurrence already mixes channels
selectively, so a static pre-recurrence gate adds little. This is the expected
tail of the transfer boundary---the masks help where a single global
channel-attention would otherwise be undifferentiated, and fade where the host
already imposes channel structure.
