...
backbones. Dropped unchanged into a GRU-based state-space forecaster, the masks
again lower error against a matched all-ones control in all 16 PEMS cells (mean
$13.8\%$, up to $26.6\%$); only on CrossFormer's two-stage attention, which
already factorises channel interaction through a learned router, do the masks
help in just 5 of 16 cells. The mechanism thus transfers wherever channel mixing
is otherwise unstructured---single-stage attention or gated recurrence---and
yields little only where the backbone imposes its own learned channel structure.
...
transfer to other channel-mixing backbones?---and answer it with matched
all-ones controls on each host. Carried unchanged into a GRU-based state-space
forecaster (\textsc{StructMamba}), the masks lower error in \emph{all 16 of 16}
PEMS cells (mean $+13.8\%$, up to $+26.6\%$)---a non-attention, recurrent host.
Only inside CrossFormer's~\cite{zhang2023crossformer} two-stage cross-dimension
attention (\textsc{SECrossformer}) do the masks help in just 5 of 16 cells:
that backbone already factorises channel interaction through a learned router,
leaving little for a static mask to add. The transfer study thus localises the
mechanism precisely: it pays off wherever channel mixing is otherwise
unstructured---global attention or gated recurrence---and yields little only
where the host imposes its own learned channel structure. We are equally
explicit on data scope: the masks help on large traffic sensor networks (both
flow and speed) but reduce to the backbone on diffuse or low-channel data
(electricity, exchange-rate, NASDAQ), reported as a first-class finding
...
\begin{itemize}
  ...
  \item \textbf{Large, causally-attributed gains on the primary backbone.}
        ...
        (Section~\ref{sec:experiments}).
  \item \textbf{A transfer study with matched controls on every host.} We carry
        the identical masks into a GRU-based state-space forecaster
        (\textsc{StructMamba}: masks beat the all-ones control in 16/16 cells,
        mean $+13.8\%$) and into CrossFormer (\textsc{SECrossformer}: 5/16)---
        the benefit follows the host's channel-mixing structure, not the
        attention softmax: strong wherever mixing is otherwise unstructured,
        small where a learned router already imposes structure
        (Section~\ref{sec:instantiations}).
  \item \textbf{An honest scope boundary.} We delimit applicability to large
        sensor networks with recoverable dependency structure (traffic flow
        \emph{and} speed), reporting the regimes (diffuse or low-channel
        data---electricity, exchange-rate, NASDAQ) where the mechanism reduces
        to its backbone, as a first-class finding rather than a footnote.
\end{itemize}
...
\input{draft_sections/framework_instantiations}
...
\input{tables/framework_backbone}
\input{tables/framework_main}


\input{draft_sections/framework_results}
